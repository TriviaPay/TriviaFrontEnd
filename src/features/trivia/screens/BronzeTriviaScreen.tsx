...
const BronzeTriviaScreen: React.FC = () => {
  // DEBUG: Log component mount
  console.log('🟢 [BRONZE TRIVIA SCREEN] Component mounted/rendered');

  // Analytics tracking
  useTrackScreenView('BronzeTriviaScreen');
  ...
  // HARDCODED MODE: This screen is always bronze mode
  const MODE: 'bronze' = 'bronze';
  ...
  } = useGetBronzeModeStatusQuery(undefined, {
    refetchOnFocus: true,
  ...
  } = useGetBronzeModeQuestionQuery(undefined, {
    refetchOnFocus: true,
    refetchOnMountOrArgChange: true,
  });

  const [submitBronzeAnswer, { isLoading: isSubmitting }] = useSubmitBronzeModeAnswerMutation();

  // Compat for legacy code
  const currentBronzeModeQuestion = questionData?.question;
  const bronzeModeStatus = statusData;
  const loading = isStatusLoading || isQuestionLoading || isSubmitting;
  const error = statusError || questionError;

  // Redux state (Legacy, keeping some for safety during transition)
  const dispatch = useDispatch<AppDispatch>();
  const {
    selectedAnswer,
    isSubmitted, // Still used for UI state? Check if we can replace with local/status
    isCorrect,
    dailyCompleted,
    // loading, // Replaced
    // error, // Replaced
    ...
    freeModeQuestions,
    freeModeStatus,
    // currentBronzeModeQuestion, // Replaced
    // bronzeModeStatus, // Replaced
  ...
  const route = useRoute<RouteProp<MainStackParamList, 'BronzeTriviaScreen'>>();
  const isFocused = useIsFocused();
  // This screen is always bronze mode - use MODE constant directly
  const routeMode = MODE; // Always bronze for this screen

  // HARDCODED MODE: Always bronze mode for this screen
  ...
  useEffect(() => {
    console.log('🔵 [BRONZE TRIVIA SCREEN] useEffect - Screen focus check', {
      isFocused,
      mode: MODE,
    });

    if (!isFocused) {
      console.log('🟡 [BRONZE TRIVIA SCREEN] Screen not focused, skipping API calls');
      return;
    }
    ...
    if (isFetchingStatusRef.current || now - lastFetchTimeRef.current < FETCH_COOLDOWN_MS) {
      console.log('🟡 [BRONZE TRIVIA SCREEN] API call skipped (already fetching or cooldown)');
      return;
    }

    console.log('🟢 [BRONZE TRIVIA SCREEN] Screen focused, mode: bronze');
    ...
    // STRICT API ISOLATION: Only call bronze mode APIs
    console.log('🔵 [BRONZE TRIVIA API] BRONZE MODE - Starting API calls');
    // Bronze mode APIs only
    // Refetch using RTK Query functions
    console.log('🔵 [BRONZE TRIVIA API] Refetching bronze mode data...');
    refetchQuestion();
    refetchStatus();
  ...
  // CRITICAL: Force state reset when new bronze mode question arrives
  // This ensures UI updates even if other useEffects have conditions that prevent them from running
  useEffect(() => {
    // This screen is always bronze mode
    if (!currentBronzeModeQuestion) {
      return;
    }

    const currentQuestionId = currentBronzeModeQuestion.question_id;
    ...
    if (currentQuestionId && currentQuestionId !== lastAnsweredQuestionId.current) {
      console.log('🟢 [BRONZE TRIVIA SCREEN] Bronze mode question changed:', {
      ...
      if (lastAnsweredQuestionId.current !== null && !currentBronzeModeQuestion.answered_at) {
        console.log('🟢 [BRONZE TRIVIA SCREEN] New question after submission - resetting UI state');
  ...
  }, [currentBronzeModeQuestion, lastAnsweredQuestionId, isSubmitted, localIsSubmitted, dispatch]);

  // After submission, fetch bronze mode status (bronze is single-question mode)
  useEffect(() => {
    console.log('🔵 [BRONZE TRIVIA SCREEN] useEffect - After submission check', {
      isSubmitted,
    ...
    if (isSubmitted && submissionResult && !nextQuestionScheduled.current) {
      ...
      console.log(
        '🔵 [BRONZE TRIVIA SCREEN] Submission detected, current question ID:',
        currentQuestionId
      );

      if (!currentQuestionId) {
        console.log('🟡 [BRONZE TRIVIA SCREEN] No current question ID, skipping');
        return;
      }

      // Skip if we already processed this question
      if (lastAnsweredQuestionId.current === currentQuestionId) {
        console.log('🟡 [BRONZE TRIVIA SCREEN] Already processed this question, skipping');
        return;
      }

      console.log('🟢 [BRONZE TRIVIA SCREEN] Scheduling status fetch...');
      nextQuestionScheduled.current = true;
      lastAnsweredQuestionId.current = currentQuestionId;

      // Bronze is a single-question mode: fetch status after submission
      const timer = setTimeout(() => {
        const run = async () => {
          try {
            console.log('🔵 [BRONZE TRIVIA API] Fetching bronze mode status after submission...');
            // Bronze is a single-question mode: do not refetch the question (it resets UI state in the slice).
            // Status is the source of truth after submission.
            await dispatch(fetchBronzeModeStatus()).unwrap();
          } finally {
            nextQuestionScheduled.current = false;
            console.log('🟢 [BRONZE TRIVIA SCREEN] Status fetch completed');
          }
        };
      ...
      return () => {
        console.log('🟡 [BRONZE TRIVIA SCREEN] Cleanup: clearing timer');
        clearTimeout(timer);
      };
    }
  }, [isSubmitted, submissionResult, currentBronzeModeQuestion, dispatch]);

  // Display previously answered questions with correct/wrong indicators
  useEffect(() => {
    // HARDCODED: Always bronze mode
    ...
    if (questionData && isAnswered) {
      ...
      if (lastAnsweredQuestionId.current === questionData.question_id) {
        logger.debug('BRONZE MODE - Skipping display logic for question just answered', 'TRIVIA', {
      ...
      logger.debug('BRONZE MODE - Loading previously answered question', 'TRIVIA', {
        question_id: questionData.question_id,
      ...
      if (userAnswer) {
        ...
        if (matchingOption) {
          ...
          engineActions.setSelectedAnswer(optionId);
          if (!isCorrect) {
            engineActions.setPreviousAnswer(optionId);
            engineActions.setPreviousAnswerCorrect(false);
          }
        } else if (['a', 'b', 'c', 'd'].includes(userAnswerText)) {
          const optionId = userAnswerText.toLowerCase();
          dispatch(setSelectedAnswer(optionId));
          engineActions.setSelectedAnswer(optionId);
          if (!isCorrect) {
            engineActions.setPreviousAnswer(optionId);
      ...
      // Mark as already answered - use engine actions
      engineActions.setAlreadyAnswered(true);
      engineActions.setLocalIsSubmitted(true);
      dispatch(setIsSubmitted(true));

      // Show congrats screen for already-answered questions (same as Silver Mode)
      if (isFocused) {
        setTimeout(() => {
      ...
      // Question not yet answered - reset states using engine
  ...
  // Get current question - always bronze mode
  const currentQuestionData = useMemo(() => {
    if (currentBronzeModeQuestion) {
      return currentBronzeModeQuestion;
    }
    return null;
  }, [currentBronzeModeQuestion]);
  ...
  // Lottie animation states - RESTORED correct/wrong animations
  ...
  // Track previous answer for display - show marks until next question
  ...
  // Store correct answer from API response (might be different format like "Avocado" instead of "a")
  ...
  const handleEngineSubmit = useCallback(
    (questionId: string | number, answer: string) => {
      submitBronzeAnswer({ question_id: Number(questionId), answer });
    },
    [submitBronzeAnswer]
  );

  // Initialize shared trivia engine - handles all quiz logic consistently
  const triviaEngine = useTriviaEngine({
    mode: 'bronze',
    currentQuestion: currentBronzeModeQuestion,
    question,
    playSoundSafely,
    trackAction,
    submissionResult,
    isSubmitted,
    modeStatus: bronzeModeStatus,
    onSubmit: handleEngineSubmit,
  });

  // Extract engine state and actions
  const { state: engineState, actions: engineActions } = triviaEngine;

  // Use engine state (fallback to local state for compatibility during migration)
  const engineSelectedAnswer = engineState.selectedAnswer || selectedAnswer;
  const engineLocalIsSubmitted = engineState.localIsSubmitted || localIsSubmitted;
  const engineAlreadyAnswered = engineState.alreadyAnswered || alreadyAnswered;
  const engineShowCongratsScreen = engineState.showCongratsScreen || showCongratsScreen;
  const enginePreviousAnswer = engineState.previousAnswer || previousAnswer;
  const enginePreviousAnswerCorrect = engineState.previousAnswerCorrect || previousAnswerCorrect;
  const engineApiCorrectAnswer = engineState.apiCorrectAnswer || apiCorrectAnswer;
  ...
  useEffect(() => {
    // Bronze screen doesn't have free mode review - skip
    if (!isFreeModeReviewMode) return;

    if (freeModeReviewTotal > 0 && freeModeReviewIndex > freeModeReviewTotal - 1) {
      setFreeModeReviewIndex(0);
    }
  }, [isFreeModeReviewMode, freeModeReviewIndex, freeModeReviewTotal]);
  ...
  // Helper function to play sound safely - uses same approach as SettingsScreen
  const playSoundSafely = useCallback(async (soundName: string, fallbackName?: string) => {
    ...
    if (!(audioManager as any).isInitialized) {
      ...
      if (typeof (audioManager as any).initializeInBackground === 'function') {
        (audioManager as any).initializeInBackground();
      } else {
        // Fallback to async initialization
        (audioManager as any).initialize().catch(() => {
    ...
    // CRITICAL: Only use AudioManager - NO fallback to prevent null path errors
    ...
    // NO FALLBACK - AudioManager is the only safe way to play sounds
    // Fallback removed completely to prevent null path errors from react-native-fs/unlink
  }, []);

  // Use engine's handleOptionSelect - replaces custom implementation
  // Engine handles all logic: validation, selection, auto-submit
  const handleOptionSelect = useCallback(
    (optionId: string) => {
      // Block if in review mode (Free Mode specific, but check for safety)
      if (isFreeModeReviewMode || showFreeModeCompletionModal) {
        logger.debug('handleOptionSelect - BLOCKED: review/completed', 'TRIVIA');
        return;
      }
      // Engine handles all the logic
      engineActions.handleOptionSelect(optionId);
    },
    [engineActions, isFreeModeReviewMode, showFreeModeCompletionModal]
  );
  ...
  useEffect(() => {
    // This screen is always bronze mode - skip free mode review logic
  ...
  // Engine handles submission automatically via handleOptionSelect
  // No need for separate handleSubmit - engine calls it internally

  // Use engine's animation handlers - they ALWAYS show congrats screen (same for all modes)
  const handleCorrectAnimationEnd = engineActions.handleCorrectAnimationEnd;
  const handleWrongAnimationEnd = engineActions.handleWrongAnimationEnd;

  // Use engine's confetti handler - ALWAYS shows congrats screen (same for all modes)
  const handleConfettiAnimationEnd = engineActions.handleConfettiAnimationEnd;
  ...
  // CRITICAL: Bronze mode needs to handle already-answered questions to show marks
  useEffect(() => {
    ...
    if (mode === 'silver' || mode === 'bronze') {
      ...
      if (statusSaysSubmitted || questionSaysSubmitted) {
        ...
        if (userAnswer) {
          const answerLower = String(userAnswer).toLowerCase().trim();
          // Map to option ID if needed
          let mappedAnswer = answerLower;
          if (!['a', 'b', 'c', 'd'].includes(mappedAnswer)) {
            const matchingOption = question?.options?.find(
              opt =>
                opt.id?.toLowerCase() === mappedAnswer || opt.text?.toLowerCase() === mappedAnswer
            );
            if (matchingOption) {
              mappedAnswer = matchingOption.id.toLowerCase();
            }
          }
          setPreviousAnswer(mappedAnswer);
    ...
    if (freeModeAnswered || hasStatusAnswered || hasUserAnswer || hasAnsweredAt) {
      ...
      // For Bronze mode, use currentBronzeModeQuestion or apiQuestion
      const userAnswerToSet = currentBronzeModeQuestion?.answered_at
        ? currentBronzeModeQuestion.fill_in_answer || selectedAnswer
        : apiQuestion.user_answer || apiQuestion.fill_in_answer || null;
      if (userAnswerToSet) {
        // Map to option ID if needed
        let mappedAnswer = userAnswerToSet.toLowerCase().trim();
        if (!['a', 'b', 'c', 'd'].includes(mappedAnswer)) {
          const matchingOption = question?.options?.find(opt => {
            const optId = opt.id.toLowerCase().trim();
            const optText = opt.text.toLowerCase().trim();
            return (
              optId === mappedAnswer ||
              optText === mappedAnswer ||
              optText.includes(mappedAnswer) ||
              mappedAnswer.includes(optText)
            );
          });
          if (matchingOption) {
            mappedAnswer = matchingOption.id.toLowerCase().trim();
          }
        }
        dispatch(setSelectedAnswer(mappedAnswer));
      }
      setLocalIsSubmitted(true);
      dispatch(setIsSubmitted(true));
      ...
      const correctAnswerToMap =
        currentBronzeModeQuestion?.correct_answer || apiQuestion.correct_answer;
      ...
      // For Bronze mode, use currentBronzeModeQuestion or apiQuestion
      const userAnswer = currentBronzeModeQuestion?.answered_at
        ? currentBronzeModeQuestion.fill_in_answer || selectedAnswer
        : apiQuestion.user_answer || apiQuestion.fill_in_answer || null;
      const answerIsCorrect =
        currentBronzeModeQuestion?.is_correct ?? apiQuestion.is_correct ?? false;

      if (userAnswer) {
        // Map to option ID if needed
        let mappedAnswer = userAnswer.toLowerCase().trim();
        if (!['a', 'b', 'c', 'd'].includes(mappedAnswer)) {
          const matchingOption = question?.options?.find(opt => {
            const optId = opt.id.toLowerCase().trim();
            const optText = opt.text.toLowerCase().trim();
    ...
    currentBronzeModeQuestion?.answered_at,
    currentBronzeModeQuestion?.is_correct,
    currentBronzeModeQuestion?.fill_in_answer,
        ...
        if (statusResult && statusResult.is_answered) {
          ...
          if (correctAnswerToMap) {
            ...
            const matchingOption = question?.options?.find(opt => {
              ...
              // Direct ID match
              if (optId === apiCorrectAnswerValue) return true;
              // Exact text match
              if (optText === apiCorrectAnswerValue) return true;

              // Only do fuzzy text match if apiValue is NOT a single letter ID
              // This prevents 'c' matching 'Africa'
              if (apiCorrectAnswerValue.length > 1) {
                if (optText.includes(apiCorrectAnswerValue) || apiCorrectAnswerValue.includes(optText)) return true;
              }
              return false;
            });
            if (matchingOption) {
          ...
          // CRITICAL: Set previous answer state from API response
          // This ensures marks (X for wrong, checkmark for correct) are shown based on API response
          // For Bronze mode, use currentBronzeModeQuestion or statusResult
          const userAnswerToSet = currentBronzeModeQuestion?.answered_at
            ? currentBronzeModeQuestion.fill_in_answer || selectedAnswer
            : statusResult.user_answer || statusResult.fill_in_answer || null;
          const answerIsCorrect =
            currentBronzeModeQuestion?.is_correct ?? statusResult.is_correct ?? false;

          if (userAnswerToSet) {
            // Map user answer to option ID if it's text
            let mappedAnswer = userAnswerToSet.toLowerCase().trim();
            if (!['a', 'b', 'c', 'd'].includes(mappedAnswer)) {
              ...
              const matchingOption = question?.options?.find(opt => {
                ...
                // Direct ID match
                if (optId === mappedAnswer) return true;
                // Exact text match
                if (optText === mappedAnswer) return true;

                // Only do fuzzy text match if mappedAnswer is NOT a single letter ID
                if (mappedAnswer.length > 1) {
                  if (optText.includes(mappedAnswer) || mappedAnswer.includes(optText)) return true;
                }
                return false;
              });
              if (matchingOption) {
            ...
            // Use API response is_correct to determine if previous answer was correct
            setPreviousAnswerCorrect(Boolean(answerIsCorrect));
            // Also set selectedAnswer for UI display
            dispatch(setSelectedAnswer(mappedAnswer));
          }
  ...
  useEffect(() => {
    ...
    if (submissionResult.correct_answer) {
      ...
      const matchingOption = question?.options?.find(opt => {
        ...
        // CRITICAL FIX: Only if NOT single letter ID
        ...
        >
          ...
          <TouchableWithoutFeedback
            onPress={() => {
              ...
              const mode = routeMode || currentMode || 'free';
              if (mode !== 'free' && alreadyAnswered && !showCongratsScreen) {
                setShowCongratsScreen(true);
              }
            }}
          >
            <View style={{ flex: 1 }}>
              ...
              {/* Fixed middle part, correct ScrollView */}
              <ScrollView
                style={{ flex: 1 }}
              ...
              >
                <View
                  style={{
                    ...
                    zIndex: showCongratsScreen ? 0 : 1, // Lower z-index when modal is open
                    elevation: showCongratsScreen ? 0 : 1, // Android elevation
                            ...
                            >
                              <Animated.View style={optionAnimation.animatedStyle}>
                                <OptionButton
                                  ...
                                  optionIndex={index}
                                  isSelected={
                                    engineSelectedAnswer?.toLowerCase() === option.id?.toLowerCase()
                                  }
                                  isSubmitted={engineLocalIsSubmitted || isSubmitted}
                                  correctAnswer={engineApiCorrectAnswer || ''} // Engine manages correct answer from API response
                                  onPress={() => handleOptionSelect(option.id)}
                                  hasAnySelection={engineSelectedAnswer !== null}
                                  ...
                                  previousAnswer={enginePreviousAnswer}
                                  previousAnswerCorrect={enginePreviousAnswerCorrect}
                                  alreadyAnswered={engineAlreadyAnswered}
                                />
                              </Animated.View>
                            </Animated.View>
                          );
              ...
              {/* Fixed Bottom Navigation - Moved OUTSIDE ScrollView */}
              {(() => {
                const statusColor = getStatusColor();
                const statusText = getStatusText();
                const isPrevDisabled = freeModeReviewIndex <= 0;
                const isNextDisabled = freeModeReviewIndex >= (freeModeReviewTotal || 0) - 1;
                ...
                return (
                  ...
                  >
                    <SoundTouchableOpacity
                      ...
                      disabled={isPrevDisabled}
                      activeOpacity={0.9}
                      style={{ opacity: isPrevDisabled ? 0.4 : 1, padding: scaleSize(8) }}
                    >
                      <Icon name="chevron-left" size={scaleSize(34)} color="#ffffff" />
                    </SoundTouchableOpacity>
                    ...
                    <SoundTouchableOpacity
                      ...
                      disabled={isNextDisabled}
                      activeOpacity={0.9}
                      style={{ opacity: isNextDisabled ? 0.4 : 1, padding: scaleSize(8) }}
                    >
                      <Icon name="chevron-right" size={scaleSize(34)} color="#ffffff" />
                    </SoundTouchableOpacity>
                  </View>
...
export default BronzeTriviaScreen;
