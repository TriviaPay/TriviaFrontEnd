...
// Import SoundManager to check notification settings
let audioManager = null;
try {
  audioManager = require('../audio/sound-manager').default;
} catch (error) {
  // SoundManager not available - will check later
}
...
class NotificationManager {
  ...
  showNotification(title, message, type = this.NOTIFICATION_TYPES.SYSTEM_ALERT, options = {}) {
    // Check if notifications are enabled - if not, don't show anything
    if (!this.areNotificationsEnabled()) {
      logger.debug(`🔕 Notifications disabled, skipping: ${title} - ${message}`, 'NOTIFICATIONS');
      return;
    }
